...
const PROBLEM_POOL = [{ contestId: 1010, index: "A", rating: 800 }];
...
wss.on("connection", (ws) => {
  ...
  // Set up ping interval to detect broken connections
  ws.isAlive = true;
  ws.on("pong", () => {
    ws.isAlive = true;
  });
  ...
  try {
    ...
    if (solved1 || solved2) {
      ...
      if (solved1 && solved2) {
        ...
        winner =
          solved1.creationTimeSeconds < solved2.creationTimeSeconds
            ? player1
            : player2;
        solveTime =
          Math.min(solved1.creationTimeSeconds, solved2.creationTimeSeconds) -
...
function endMatch(matchId, winner, solveTime) {
  ...
  console.log(`Match ${matchId} ended. Winner: ${winner}, Time: ${solveTime}s`);
  console.log(`Players: ${match.player1} vs ${match.player2}`);
  ...
  // Send to player 1 with opponent info
  const dataForPlayer1 = {
    ...matchEndData,
    opponent: match.player2,
  };
  console.log("Sending to", match.player1, ":", dataForPlayer1);
  sendMessage(match.player1, dataForPlayer1);

  // Send to player 2 with opponent info
  const dataForPlayer2 = {
    ...matchEndData,
    opponent: match.player1,
  };
  console.log("Sending to", match.player2, ":", dataForPlayer2);
  sendMessage(match.player2, dataForPlayer2);
